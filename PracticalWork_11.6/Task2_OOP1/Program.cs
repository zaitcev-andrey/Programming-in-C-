                ...
                case "2":
                    {
                        ...
                        Console.WriteLine("1) Протестируем работу с получением и изменением ФИО");
                        manager.PrintClientFIO(clients[0]);
                        manager.SetClientFio(clients[0]);
                        manager.PrintClientFIO(clients[0]);
                        Console.WriteLine("2) Протестируем работу с получением и изменением номера телефона");
                        Console.WriteLine($"Номер телефона: {manager.GetClientNumber(clients[0])}");
                        manager.SetClientNumber(clients[0]);
                        Console.WriteLine($"Номер телефона: {manager.GetClientNumber(clients[0])}");
                        Console.WriteLine("3) Протестируем работу с получением и изменением данных паспорта");
